...
// STATIC VERSION: All data hardcoded, no transforms, no CSV loading
...
// ═══════════════════════════════════════════════════════════════
// 4 INDEPENDENT LABEL OFFSET RATIOS — user tuned
// ═══════════════════════════════════════════════════════════════
...
const overallData = histData.map(d => ({
...
histData.forEach(d => {
  histStacked.push({
    ...
    bin_end: d.bin_end,
    group: 'single-genre',
    y_density: d.single_count / totalAll * 100,
  ...
  histStacked.push({
    ...
    bin_end: d.bin_end,
    group: 'multi-genre',
    y_density: d.multi_count / totalAll * 100,
...
function makeCurve(m, s, c) {
  ...
  for (let x = 5; x <= 100; x++) {
    ...
    p.push({
      x: x,
      y_density: d * c.densityScale * 100,
...
let singlePeakDensity = gaussianPDF(SINGLE_MEAN, SINGLE_MEAN, SINGLE_STD) * SINGLE_DENSITY_SCALE * 100;
let singlePeakCount   = gaussianPDF(SINGLE_MEAN, SINGLE_MEAN, SINGLE_STD) * SINGLE_COUNT * SINGLE_COUNT_SCALE;
let multiPeakDensity  = gaussianPDF(MULTI_MEAN,  MULTI_MEAN,  MULTI_STD)  * MULTI_DENSITY_SCALE * 100;
...
// ═══════════════════════════════════════════════════════════════
// SPEC 2B — STACKED (two colors, stack: true)
// ═══════════════════════════════════════════════════════════════
